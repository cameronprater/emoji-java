...
public class EmojiParser {
     ...
     * (between 2 ':').
     * <p>
     * Example: <code>😄</code> will be replaced by <code>:smile:</code>
     * <p>
     * When a fitzpatrick modifier is present with a PARSE action, a "|" will be appended to the alias, with the
     * fitzpatrick type.
     * <p>
     * Example: <code>👦🏿</code> will be replaced by <code>:boy|type_6:</code>
     ...
     * Example: <code>👦🏿</code> will be replaced by <code>:boy:</code>
     * <p>
     * When a fitzpatrick modifier is present with a IGNORE action, the modifier will be ignored.
     * Example: <code>👦🏿</code> will be replaced by <code>:boy:🏿</code>
     *
     * @param input             the string to parse
     * @param fitzpatrickAction the action to apply for the fitzpatrick modifier
     * @return the string with the emojis replaced by their alias.
     */
     ...
     * <code>&amp;#128516;</code> will be replaced by <code>😄</code>
    ...
    static AliasCandidate getAliasAt(String input, int start) {
    ...
    static AliasCandidate getHtmlEncodedEmojiAt(String input, int start) {
        ...
        do {
            ...
            Emoji foundEmoji = EmojiManager.EMOJI_TRIE.getEmoji(chars, charsIndex);
            if (foundEmoji != null) {
                longestEmoji = foundEmoji;
                longestCodePointEnd = codePointEnd;
            }
            codePointStart = codePointEnd + 1;
        } while (input.length() > codePointStart + 4 && input.charAt(codePointStart) == '&' && input.charAt(codePointStart + 1) == '#' &&
            charsIndex < chars.length && !EmojiManager.EMOJI_TRIE.isEmoji(chars, 0, charsIndex).impossibleMatch());
    ...
    /**
     * Generates a list UnicodeCandidates found in input string. A UnicodeCandidate is created for every unicode
     * emoticon found in input string, additionally if Fitzpatrick modifier follows the emoji, it is included in
     * UnicodeCandidate. Finally, it contains start and end index of unicode emoji itself (WITHOUT Fitzpatrick modifier
     * whether it is there or not!).
     *
     * @param input String to find all unicode emojis in
     * @return List of UnicodeCandidates for each unicode emote in text
     */
    static List<UnicodeCandidate> getUnicodeCandidates(String input) {
     ...
     */
    static UnicodeCandidate getNextUnicodeCandidate(char[] chars, int start) {
     ...
     */
    static int getEmojiEndPos(char[] text, int startPos) {
        int best = -1;
