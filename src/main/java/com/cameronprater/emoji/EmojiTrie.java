...
public class EmojiTrie {
    ...
    public Emoji getEmoji(String unicode) {
        return getEmoji(unicode.toCharArray(), unicode.length());
    }

    Emoji getEmoji(char[] sequence, int end) {
        final int START = 0;
        if (START > end || end > sequence.length) {
            throw new ArrayIndexOutOfBoundsException("start " + 0 + ", end " + end + ", length " + sequence.length);
        }
        Node tree = root;
        for (int i = 0; i < end; i++) {
            if (!tree.hasChild(sequence[i])) {
